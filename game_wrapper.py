...
class Wrapper:
    ...
    def init(self):
        ...
        self.pipe_out_file = os.path.join(tmpdir, "out")
        self.pipe_in_file = os.path.join(tmpdir, "in")
        os.mkfifo(self.pipe_out_file)
        os.mkfifo(self.pipe_in_file)

        def listen_pipe_in():
            pipe_in = open(self.pipe_in_file, 'r', 1)
            buffer = ""
        ...
        with open(tmp_file, "w") as f:
            f.write("pipe_in_file = \"{}\"; pipe_out_file = \"{}\"; target = \"111\"; draw_tiles = {}; frame_gap = {};"
                    ...
                    .format(os.path.abspath(self.pipe_in_file), os.path.abspath(self.pipe_out_file),
                            "true" if self.draw_tiles else "false", self.frame_gap,
                            os.path.abspath(config.PLUGIN_FILE))
                    ) 

        args = [config.FCEUX_PATH, '--xscale 2', '--yscale 2', '-f 0', "--nogui", "--sound 0",
                '--loadlua', tmp_file, config.ROM_FILE,
                '>/dev/null', '2>/dev/null', '&'
                'ps', 'o', 'pid,pgid,comm']
        print(' '.join(args))
        self.subprocess = subprocess.Popen(' '.join(args), shell=True, stdout=subprocess.PIPE)
        procs = self.subprocess.communicate()[0].decode()
        pgid = int(re.findall(r" {0,}%d {1,}(\d{1,}) sh\n" % self.subprocess.pid, procs)[-1])
        self.fecux_pid = int(re.findall(r" {0,}(\d{1,}) {1,}%d fceux\n" % pgid, procs)[-1])
        print("Capture Fecux pid: {}".format(self.fecux_pid))
        if self.subprocess.returncode != 0:
            raise ValueError("Failed to launch Fceux")
    ...
    def step(self, action):
        ...
        return self.screen.copy(), reward, over
    ...
    def notify(self, message):
        with self.locker:
            with open(self.pipe_out_file, 'w') as f:
                f.write(message + '\n')
